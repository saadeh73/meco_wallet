    ...
    try {
      ...
      for (let i = 1; i < 10; i++) {
        ...
        try {
          ...
          // ✅ لا نتوقف عند أول حساب فارغ. السبب: addAccountFromPrivateKey في
          // store.js بتسمح باستيراد مفتاح خاص خارجي غير مشتق من العبارة السرية
          // في أي فهرس تسلسلي — لو كان موجودًا في المنتصف، أي حساب حقيقي مشتق
          // من العبارة السرية بعده كان هيضيع تمامًا من الاستكشاف عند تسجيل
          // الدخول من جهاز جديد. نفحص كل الفهارس العشرة دايمًا لضمان استرجاع
          // كل حساب مشتق من العبارة السرية بغض النظر عن ترتيبه.
        } catch (e) {
          console.warn(`Error scanning account ${i}:`, e);
          // ✅ نفس السبب: لا نوقف الفحص كامل بسبب خطأ عابر في فهرس واحد فقط
        }
      }
