...
// ── Pie Chart ─────────────────────────────────────────────────────────────────
function PieChart({ slices, total, primaryColor }) {
  if (!slices?.length || total === 0) return (
    <View style={[S.pieWrap, { width:PIE_SIZE, height:PIE_SIZE }]}>
      <Svg width={PIE_SIZE} height={PIE_SIZE}>
        <Circle cx={PIE_CX} cy={PIE_CY} r={PIE_R} fill="rgba(128,128,128,0.15)" />
        <Circle cx={PIE_CX} cy={PIE_CY} r={50} fill="transparent" />
      </Svg>
    </View>
  );
    ...
    <View style={{ width:PIE_SIZE, height:PIE_SIZE, alignItems:'center', justifyContent:'center' }}>
      <Svg width={PIE_SIZE} height={PIE_SIZE}>
        <G>
          ...
          <Circle cx={PIE_CX} cy={PIE_CY} r={52} fill="transparent" />
        </G>
      </Svg>
    </View>
  );
...
export default function PortfolioScreen() {
  ...
  const C = {
    bg:      isDark ? '#0A0A0F' : '#F2F3F7',
    card:    isDark ? '#1A1A2E' : '#FFFFFF',
    card2:   isDark ? '#252540' : '#F4F5FF',
  ...
  const [selected,    setSelected]    = useState(null); // العنصر المحدد في الرسم الدائري
  ...
  // Pie slices — فقط الأصول التي لها قيمة حقيقية
    ...
    <SafeAreaView style={[S.root, { backgroundColor: C.bg }]}>
      ...
      {/* Header */}
       <View style={[S.header, { backgroundColor: C.card }]}>
            <TouchableOpacity style={[S.backBtn, { backgroundColor: C.card2 }]} onPress={() => navigation.goBack()}>
      ...
      {loading ? (
        <View style={S.loadCenter}>
          ...
          <Text style={[{ fontSize:14, marginTop:12 }, { color: C.muted }]}>{t('loading')}</Text>
        </View>
      ) : (
        <Animated.ScrollView
        ...
        >
          {/* ── إجمالي المحفظة ── */}
          <View style={[S.totalCard, { backgroundColor: C.card }]}>
          ...
          {/* ── الرسم الدائري ── */}
          {pieSlices.length > 0 && (
            <View style={[S.pieCard, { backgroundColor: C.card }]}>
              <Text style={[S.sectionTitle, { color: C.text }]}>{t('asset_distribution', 'توزيع الأصول')}</Text>
              <View style={S.pieRow}>
                {/* الرسم الدائري */}
                <View style={S.pieContainer}>
                  <PieChart slices={pieSlices} total={totalUSD} primaryColor={primaryColor} />
                  {/* المبلغ في المنتصف */}
                ...
                <View style={S.legend}>
                  {pieSlices.slice(0, 6).map((slice, i) => (
                    <TouchableOpacity
                      key={slice.symbol}
                      style={[S.legendItem, selected?.symbol === slice.symbol && { backgroundColor: slice.color+'15', borderRadius:8 }]}
          ...
          {/* ── قائمة الأصول مع P&L ── */}
          <View style={[S.assetsCard, { backgroundColor: C.card }]}>
              ...
              return (
                ...
                >
                  ...
                  <View style={S.assetRight}>
                    ...
                    <View style={S.pnlRow}>
                      <Ionicons name={assetUp?'caret-up':'caret-down'} size={10} color={assetUp?C.success:C.error} />
          ...
          {/* ── ملخص P&L ── */}
          {totalUSD > 0 && (
            <View style={[S.pnlCard, { backgroundColor: C.card }]}>
              ...
              <View style={S.pnlGrid}>
                {[
                  ...
                  <View key={item.label} style={[S.pnlItem, { backgroundColor: C.card2 }]}>
...
const S = StyleSheet.create({
  root:        { flex:1 },
  header:      { flexDirection:'row', justifyContent:'space-between', alignItems:'center', paddingHorizontal:20, paddingVertical:16, borderBottomLeftRadius:24, borderBottomRightRadius:24, shadowColor:'#000', shadowOpacity:0.05, shadowRadius:8, elevation:4 },
  ...
  scroll:      { padding:16 },
  totalCard:   { borderRadius:24, padding:24, alignItems:'center', marginBottom:14, shadowColor:'#000', shadowOffset:{width:0,height:4}, shadowOpacity:0.06, shadowRadius:12, elevation:4 },
  totalLabel:  { fontSize:13, fontWeight:'500', marginBottom:8 },
  totalAmount: { fontSize:40, fontWeight:'900', letterSpacing:-1, marginBottom:10 },
  changePill:  { flexDirection:'row', alignItems:'center', gap:6, paddingHorizontal:14, paddingVertical:7, borderRadius:20 },
  changeText:  { fontSize:14, fontWeight:'700' },
  pieCard:     { borderRadius:24, padding:20, marginBottom:14, shadowColor:'#000', shadowOffset:{width:0,height:4}, shadowOpacity:0.06, shadowRadius:12, elevation:4 },
  ...
  pieCenterAmt:{ fontSize:13, fontWeight:'800' },
  pieCenterLabel:{ fontSize:10, marginTop:2 },
  legend:      { flex:1, gap:8 },
  legendItem:  { flexDirection:'row', alignItems:'center', gap:8, padding:4 },
  legendDot:   { width:10, height:10, borderRadius:5 },
  legendInfo:  { flex:1 },
  legendSym:   { fontSize:13, fontWeight:'700' },
  legendPct:   { fontSize:11, marginTop:1 },
  assetsCard:  { borderRadius:24, padding:20, marginBottom:14, shadowColor:'#000', shadowOffset:{width:0,height:4}, shadowOpacity:0.06, shadowRadius:12, elevation:4 },
  assetRow:    { flexDirection:'row', alignItems:'center', paddingVertical:14, gap:12 },
  assetIcon:   { width:44, height:44, borderRadius:13, justifyContent:'center', alignItems:'center' },
  assetInfo:   { flex:1 },
  assetSym:    { fontSize:15, fontWeight:'700' },
  assetAmt:    { fontSize:11, marginTop:2 },
  assetRight:  { alignItems:'flex-end' },
  assetUSD:    { fontSize:15, fontWeight:'700' },
  pnlRow:      { flexDirection:'row', alignItems:'center', gap:3, marginTop:3 },
  pnlText:     { fontSize:11, fontWeight:'600' },
  pnlUSD:      { fontSize:10 },
  empty:       { alignItems:'center', paddingVertical:40, gap:12 },
  emptyText:   { fontSize:15 },
  pnlCard:     { borderRadius:24, padding:20, marginBottom:14, shadowColor:'#000', shadowOffset:{width:0,height:4}, shadowOpacity:0.06, shadowRadius:12, elevation:4 },
  pnlGrid:     { flexDirection:'row', flexWrap:'wrap', gap:10 },
  pnlItem:     { width:(width-32-20-10)/2, padding:16, borderRadius:16 },
  pnlItemLabel:{ fontSize:11, marginBottom:6 },
  pnlItemValue:{ fontSize:16, fontWeight:'800' },
});
