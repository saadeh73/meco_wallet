...
import { getTransactionLog } from '../services/transactionService'; // ✅ اسم صحيح
...
export default function TransactionHistoryScreen() {
  ...
  // ✅ استخدام activeAccount كباقي الشاشات
  ...
  const loadTransactions = async () => {
    try {
      ...
      // ── دمج البيانات ──────────────────────────────────────────────────────
      // ✅ استخدام signature كمفتاح موحد
      const mergedMap = new Map();
      onChain.forEach(tx => {
        if (tx.signature) mergedMap.set(tx.signature, tx);
      });

      localLog.forEach(tx => {
        // ✅ دعم كلا الحقلين signature و transactionSignature
        const key = tx.signature || tx.transactionSignature;
        if (!key) return;
        ...
        mergedMap.set(key, {
          ...existing,
          ...tx,
          signature: key, // ✅ توحيد اسم الحقل
          type: tx.type === 'swap'    ? 'swap'
              : tx.from === publicKey ? 'send'
              : tx.to   === publicKey ? 'receive'
              : tx.type || existing.type || 'onchain',
        });
      });
      ...
      all.sort((a, b) => {
        const timeA = a.timestamp ? new Date(a.timestamp).getTime() : (a.blockTime || 0) * 1000;
        const timeB = b.timestamp ? new Date(b.timestamp).getTime() : (b.blockTime || 0) * 1000;
        return timeB - timeA;
      });
      ...
      all.forEach(tx => {
        ...
        const target   =
          currency === 'sol'  ? newStats.sol  :
          currency === 'meco' ? newStats.meco :
          currency === 'usdt' ? newStats.usdt :
          currency === 'usdc' ? newStats.usdc :
          newStats.other;
        ...
        if (isSuccess) {
          if (tx.type === 'send')    target.totalSent     += tx.amount || 0;
          if (tx.type === 'receive') target.totalReceived += tx.amount || 0;
          // ✅ إحصاء معاملات swap أيضاً
          if (tx.type === 'swap')    target.count         += 1;
          target.count += tx.type !== 'swap' ? 1 : 0;
          if (currency === 'sol')   newStats.sol.totalFees += tx.fee || 0;
        }
      });
  ...
  const openExplorer = async (signature) => {
    if (!signature) { Alert.alert(t('error'), t('no_transaction_id')); return; }
    const url = `https://solscan.io/tx/${signature}`;
    try {
      const supported = await Linking.canOpenURL(url);
      if (supported) await Linking.openURL(url);
      else Alert.alert(t('error'), t('cannot_open_link'));
    } catch {
  ...
  const formatDate = useCallback((timestamp, blockTime) => {
    try {
      const date = timestamp ? new Date(timestamp) : blockTime ? new Date(blockTime * 1000) : null;
      if (!date || isNaN(date.getTime())) return t('unknown_date');
  ...
  const getStatusInfo = useCallback((tx) => {
    if (tx.err) return { color: colors.error,   label: t('failed'),  bg: colors.errorLight   };
  ...
  const renderStats = () => {
    const statsToShow = [
      { currency: 'SOL',  data: stats.sol,  color: primaryColor    },
      { currency: 'MECO', data: stats.meco, color: colors.warning  },
      { currency: 'USDT', data: stats.usdt, color: colors.success  },
      { currency: 'USDC', data: stats.usdc, color: colors.info     },
    ].filter(s => s.data.count > 0 || s.data.totalSent > 0 || s.data.totalReceived > 0);

    if (statsToShow.length === 0) return null;

    return (
      <View style={styles.statsSection}>
        <Text style={[styles.statsTitle, { color: colors.textSecondary }]}>{t('summary')}</Text>
            ...
            <View key={index} style={[styles.statCard, { backgroundColor: colors.card, borderColor: item.color + '30' }]}>
              <View style={[styles.statHeader, { backgroundColor: item.color + '15' }]}>
                ...
                <View style={[styles.statCountBadge, { backgroundColor: item.color }]}>
                  <Text style={styles.statCountText}>{item.data.count}</Text>
                </View>
              </View>
              <View style={styles.statDetails}>
                <View style={styles.statRow}>
                  ...
                  <Text style={[styles.statValue, { color: colors.error }]}>{item.data.totalSent.toFixed(4)}</Text>
                </View>
                <View style={styles.statRow}>
                  ...
                  <Text style={[styles.statValue, { color: colors.success }]}>{item.data.totalReceived.toFixed(4)}</Text>
                </View>
              </View>
            </View>
          ))}
      ...
      <View style={styles.modalOverlay}>
        <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
          ...
          <View style={styles.modalHeader}>
            ...
            <TouchableOpacity onPress={() => setModalVisible(false)} style={[styles.closeButton, { backgroundColor: colors.background }]}>
            ...
            <>
              <View style={[styles.modalTypeContainer, { backgroundColor: getTransactionType(selectedTx).bg }]}>
                <View style={[styles.modalIconLarge, { backgroundColor: colors.card }]}>
                  <Ionicons name={getTransactionType(selectedTx).icon} size={36} color={getTransactionType(selectedTx).color} />
                </View>
                ...
                <View style={[styles.modalDetails, { backgroundColor: colors.background }]}>
                  ...
                  {selectedTx.signature && (
                    <View style={styles.detailRow}>
                      ...
                      <TouchableOpacity style={styles.detailValueContainer} onPress={() => copyToClipboard(selectedTx.signature, t('signature_copied'))}>
                  ...
                  {/* ✅ عرض from/to في الإرسال والاستلام */}
                  {selectedTx.from && (
                    <View style={styles.detailRow}>
                      <Text style={[styles.detailLabel, { color: colors.textSecondary }]}>{t('from')}</Text>
                      <TouchableOpacity style={styles.detailValueContainer} onPress={() => copyToClipboard(selectedTx.from, t('address_copied'))}>
                        <Text style={[styles.detailValue, { color: colors.text }]} numberOfLines={1}>
                          {selectedTx.from.slice(0, 8)}...{selectedTx.from.slice(-4)}
                        </Text>
                        <Ionicons name="copy-outline" size={16} color={primaryColor} />
                      </TouchableOpacity>
                    </View>
                  ...
                  {selectedTx.to && (
                    <View style={styles.detailRow}>
                      ...
                      <TouchableOpacity style={styles.detailValueContainer} onPress={() => copyToClipboard(selectedTx.to, t('address_copied'))}>
                        <Text style={[styles.detailValue, { color: colors.text }]} numberOfLines={1}>
                          {selectedTx.to.slice(0, 8)}...{selectedTx.to.slice(-4)}
                        </Text>
                        <Ionicons name="copy-outline" size={16} color={primaryColor} />
                      </TouchableOpacity>
                    </View>
                  ...
                  {/* ✅ إصلاح عرض الرسوم — تحويل من Lamports إلى SOL */}
                  {selectedTx.fee > 0 && (
                    <View style={styles.detailRow}>
                      <Text style={[styles.detailLabel, { color: colors.textSecondary }]}>{t('fee')}</Text>
                      <Text style={[styles.detailValue, { color: colors.text }]}>
                        {(selectedTx.fee / 1e9).toFixed(6)} SOL
                      </Text>
                    </View>
                  )}
  ...
  const renderItem = ({ item, index }) => {
    ...
    return (
      <TouchableOpacity
        style={[styles.itemContainer, { backgroundColor: colors.card }, index === 0 && { marginTop: 0 }]}
      ...
      >
        ...
        <View style={styles.itemContent}>
          <View style={styles.itemTopRow}>
            <Text style={[styles.itemTitle, { color: colors.text }]} numberOfLines={1}>{txType.label}</Text>
            {item.amount && (
        ...
        <Ionicons name="chevron-forward" size={20} color={colors.textSecondary} style={styles.itemArrow} />
      </TouchableOpacity>
    );
  };
  ...
  const renderFilters = () => {
    const filters = [
      { key: 'all',     label: t('all'),      icon: 'apps',             color: primaryColor    },
      { key: 'receive', label: t('received'),  icon: 'arrow-down',       color: colors.success  },
      { key: 'send',    label: t('sent'),      icon: 'arrow-up',         color: colors.error    },
      { key: 'swap',    label: t('swapped'),   icon: 'swap-horizontal',  color: colors.info     },
    ];
    return (
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filtersContainer}>
        {filters.map((filter) => {
          ...
          return (
            <TouchableOpacity
              key={filter.key}
              style={[styles.filterChip, isActive ? { backgroundColor: filter.color } : { backgroundColor: colors.card, borderColor: colors.border }]}
              onPress={() => setActiveFilter(filter.key)}
            >
              <Ionicons name={filter.icon} size={14} color={isActive ? '#FFF' : colors.textSecondary} style={{ marginRight: 6 }} />
              <Text style={[styles.filterText, { color: isActive ? '#FFF' : colors.textSecondary }]}>{filter.label}</Text>
            </TouchableOpacity>
          );
        })}
      </ScrollView>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.card }]}>
        ...
        <TouchableOpacity onPress={onRefresh} style={[styles.refreshBtn, { backgroundColor: colors.background }]}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListHeaderComponent={() => (
            <>
              {transactions.length > 0 && renderStats()}
              {renderFilters()}
            </>
          )}
...
const styles = StyleSheet.create({
  ...
  statsTitle:     { fontSize: 13, fontWeight: '600', marginBottom: 12, paddingHorizontal: 20, textTransform: 'uppercase', letterSpacing: 1 },
  statsContainer: { paddingHorizontal: 16, paddingBottom: 16, gap: 12 },
  statCard: {
    width: 160, padding: 14, borderRadius: 20, borderWidth: 1,
    elevation: 2, shadowColor: '#000', shadowOpacity: 0.03, shadowRadius: 5,
  },
  statHeader:     { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 8, borderRadius: 10, marginBottom: 12 },
  statCurrency:   { fontSize: 15, fontWeight: '800' },
  statCountBadge: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 10 },
  statCountText:  { color: '#FFF', fontSize: 11, fontWeight: '700' },
  statDetails:    { gap: 8 },
  statRow:        { flexDirection: 'row', alignItems: 'center', gap: 8 },
  statArrow:      { width: 22, height: 22, borderRadius: 6, justifyContent: 'center', alignItems: 'center' },
  statLabel:      { fontSize: 12, fontWeight: '500', flex: 1 },
  statValue:      { fontSize: 13, fontWeight: '700' },

  filtersContainer: { paddingHorizontal: 16, paddingBottom: 16, gap: 10 },
  filterChip: {
  ...
  itemLeftSection: { width: 48, height: 48, borderRadius: 16, justifyContent: 'center', alignItems: 'center', marginRight: 14 },
  itemContent:     { flex: 1 },
  ...
  itemDateContainer: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  itemDate:        { fontSize: 12, fontWeight: '500' },
  itemStatusBadge: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8 },
  ...
  modalContent:   { borderTopLeftRadius: 32, borderTopRightRadius: 32, paddingHorizontal: 24, paddingTop: 12, paddingBottom: 40, maxHeight: '80%' },
  ...
  closeButton:    { width: 36, height: 36, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  modalTypeContainer: { alignItems: 'center', padding: 24, borderRadius: 24, marginBottom: 24 },
  modalIconLarge: { width: 80, height: 80, borderRadius: 40, justifyContent: 'center', alignItems: 'center', marginBottom: 16, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 10, elevation: 5 },
  modalTypeLabel: { fontSize: 16, fontWeight: '700', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 },
  modalAmountLarge: { fontSize: 28, fontWeight: '800' },
  modalDetailsScroll: { maxHeight: 280 },
  modalDetails:   { borderRadius: 20, padding: 8, marginBottom: 24 },
  ...
  detailValueContainer: { flexDirection: 'row', alignItems: 'center', gap: 10, flex: 1, justifyContent: 'flex-end' },
  detailValue:    { fontSize: 14, fontWeight: '600' },
  statusBadgeLarge: { paddingHorizontal: 14, paddingVertical: 6, borderRadius: 10 },
  modalButton:    { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 18, borderRadius: 20, gap: 10, shadowColor: '#000', shadowOpacity: 0.15, shadowRadius: 10, elevation: 5 },
