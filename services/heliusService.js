...
// ✅ خريطة شاملة لجميع العملات المعروفة — تُستخدم في سجل المعاملات
...
class LRUCache {
  ...
  get(key) {
    ...
    if (Date.now() - entry.timestamp > this.maxAge) { this.cache.delete(key); return null; }
    return entry.value;
  }
  ...
  try {
    ...
    if (tokenSymbol === 'MECO') {
      try {
        ...
        console.warn('⚠️ MECO DexScreener failed');
      }
      CACHE.prices.set(tokenSymbol, 0);
      return 0;
    }
    ...
    const endpoints = [
      { url: `https://api.jup.ag/price/v2?ids=${mint}`,     parser: (d) => d?.data?.[mint]?.price },
      { url: `https://price.jup.ag/v6/price?ids=${mint}`,   parser: (d) => d?.data?.[mint]?.price },
    ];
    ...
    for (const { url, parser } of endpoints) {
      try {
        ...
        if (res.ok) {
          const data  = await res.json();
          const price = parseFloat(parser(data));
        ...
        console.warn(`⚠️ Jupiter price failed for ${tokenSymbol}:`, e.message);
      }
    }

    return 0;
  } catch (error) {
    console.error(`❌ getTokenMarketPrice error for ${tokenSymbol}:`, error);
    return 0;
  }
};
...
export async function getTokenBalance(mintAddress, forceRefresh = false, address = null) {
  try {
    ...
    if (!forceRefresh && cache !== null && cache !== undefined) return cache;
...
export async function sendSolTransaction(fromKeypair, toAddress, amount) {
    ...
    for (const sig of signatures) {
      try {
        ...
        for (const ix of instructions) {
          ...
          if (ix.program === 'spl-token' && (ix.parsed?.type === 'transfer' || ix.parsed?.type === 'transferChecked')) {
            ...
            if (destIndex !== -1) {
              ...
              if (tokenData?.owner) toOwner = tokenData.owner;
              if (tokenData?.uiTokenAmount) {
            ...
            if (exactAmount <= 0) {
              let decimals = 9;
              if (parsedInfo.tokenAmount?.decimals) decimals = parsedInfo.tokenAmount.decimals;
              else {
                const tData = postToken.find(t => t.mint === mint);
                if (tData?.uiTokenAmount) decimals = tData.uiTokenAmount.decimals;
              }
        ...
        if (!found) {
          ...
          let amount = 0, tokenSymbol = 'SOL', mint = null, type = null, otherParty = null, isTokenTx = false;
          ...
          for (const post of userPostTokens) {
            ...
            if (Math.abs(delta) > 0.000001) {
              ...
              if (delta > 0) {
                type   = 'receive';
                amount = delta;
                const senderToken = preToken.find(t =>
                  t.mint === mint && t.owner !== pubKeyStr &&
                ...
                const receiverToken = postToken.find(t =>
                  t.mint === mint && t.owner !== pubKeyStr &&
          ...
          if (!isTokenTx) {
            ...
            if (delta > 0.00001) {
              ...
              type = 'send'; amount = Math.abs(delta);
          ...
          if (amount > 0 && type) {
            transactions.push({
              ...
              amount, token: tokenSymbol, mint, type,
...
export default { getConnection: () => rpcManager.getConnection() };
